...
const MsgNotificationItem = ({ smssubjectList }) => {
  const { message, id, date } = smssubjectList;
  ...
  function directToMail() {
    ...
    SmsApi.smsAssign(data)
      .subscribe(res => {

      })

    // history.push("/agentMessagePage");
  }
...
export default MsgNotificationItem;

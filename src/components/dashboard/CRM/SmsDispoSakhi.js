...
import SmsDispoDetails from "./SmsDispoDetails";
import SmsCallerDetail from "./SmsCallerDetail";
import SmsEmergencyScreen from "./SmsEmergencyScreen";
import SmsGuidenceCouncling from "./SmsGuidenceCouncling";
...
const SmsDispoSakhi = (props) => {
    ...
    const [savecalldetail, setCallerDetail] = useState({
        callType: '', incidentdate: moment(date, dateFormat)._i, incidentTime: moment(timedefault, timeFormat)._i,
        smsOption: '', message: ''
    });
    ...
    function SaveOnCallerDetail() {
        var data = {
            // id: localStorage.getItem("smsMongoId"),
        ...
        SmsDispositionApi.saveSmsCallDetails(data)
            .subscribe(response => {
                setCallerDetail({
                ...
                localStorage.setItem("smsflage", 1)
            })

        // Sms Replay with payloads
        let dispostr = localStorage.getItem("callForSms").toString();
        var disposubStr = dispostr.substr(2, 12);

        var smsdata = {
            caseId: localStorage.getItem("getSmsByCaseId"),
            message: savecalldetail.message,
            number: disposubStr
        }
        SmsDispositionApi.sendDispoSms(smsdata)
            .subscribe(response => {
            })
        // Sms Replay with payloads
    ...
    return (
        <Auxiliary>
                ...
                <Col span={24}>
                    <Row>
                        ...
                        <Col xl={5} lg={12} md={12} sm={12} xs={24}>
                            <h2 className="gx-text-uppercase gx-text-white gx-font-weight-bold gx-fnd-title">Sms Disposition</h2>
            ...
            <h1 className="gx-text-orange">CaseId: {localStorage.getItem("getSmsByCaseId")}</h1>
            <h1 className="gx-text-orange">Recived Date: {localStorage.getItem("smsRecivedDate")}</h1>
                ...
                <Col span={24}>
                    <Row>
                        ...
                        <Col xl={24} lg={12} md={12} sm={12} xs={24}>
                            <SmsDispoDetails maildetail={savecalldetail} />
            ...
            <Form
                ...
                {typeofCall.callType == "Information"
                    ? <Col xl={24} lg={12} md={12} sm={12} xs={24}>
                        ...
                        <Widget styleName={`ant-col gx-bg-white `}>
                            <SmsCallerDetail InformationScreen={localStorage.getItem("getSmsByCaseId")} />
                        </Widget>
                    </Col>
                    : <null />
                }
                ...
                {typeofCall.callType == "Emergency"
                    ? <Col xl={24} lg={12} md={12} sm={12} xs={24}>
                        ...
                        <Widget styleName={`ant-col gx-bg-white `}>
                            <SmsEmergencyScreen EmergencyScreen={localStorage.getItem("getSmsByCaseId")} />
                        </Widget>
                    </Col>
                    : <null />
                }
                ...
                {typeofCall.callType == "Counselling  & Guidance"
                    ? <Col xl={24} lg={24} md={12} sm={12} xs={24}>
                        ...
                        <Widget styleName={`ant-col gx-bg-white `}>
                            <SmsGuidenceCouncling GuidenceScreen={localStorage.getItem("getSmsByCaseId")} />
                        </Widget>
                    </Col>
                    : <null />
                }
...
<h2 className="gx-text-white" >PERPETRATOR DETAILS</h2>
    <Widget styleName={`ant-col gx-bg-white `}>
        <PerpetratorDetails/>
                ...
                {/* Remark to send SMS */}
                <Col xl={12} lg={12} md={12} sm={12} xs={24}>
                    <h2 className="gx-text-white" >Remark to Send SMS</h2>
                    <FormItem>
                        <TextArea rows={4}
                            id="message" name="message" placeholder="Remarks"
                            value={handleChangeSakhi.value} onChange={handleChangeSakhi}
                        />
                    </FormItem>
                </Col>
